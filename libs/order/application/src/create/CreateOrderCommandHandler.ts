...
export class CreateOrderCommandHandler implements CommandHandler<CreateOrderCommand> {
  ...
  async execute(command: CreateOrderCommand) {
    const { id, items, subtotal, total, dateCreated, status, isDelayed } = command.payload;

    return this.orderCreator.execute(id, items, subtotal, total, dateCreated, status, isDelayed);
  }
}
